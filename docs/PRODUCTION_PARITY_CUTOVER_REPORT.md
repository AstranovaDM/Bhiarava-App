...
**Lineage:** tip descends from `44b09b6` (Phase J) → prior tip `bdcfb75` → this parity wave  
...
| Surface | Score | Notes |
|---------|------:|-------|
| Admin web | **92%** | Dashboard, projects DRAFT→workspace setup (plot types/pricing/amenities/visibility), layouts SVG editor, CRM lists, lead conversion chain, customer onboarding wizard, finance lists, **receipt detail + A4 print**, registrations/resale/agents/reports/settings/audit/danger. Media/phases/blocks dense editors thinner than MAIN. |
| Agent web | **88%** | Home, projects+plot reserve, leads, customers+onboarding, visits, reservations, bookings, collections, commissions, documents, notifications, profile — all live API / ownership scoped. |
...
| Onboarding wizards | **90%** | Project: create DRAFT → workspace setup tabs. Customer: 4-step wizard. Lead conversion chain live. |
| In-app notifications | **90%** | List/unread/mark one/mark all/linked href; emitters on payment+booking. Provider adapters stubbed. |
| Email/SMS/WhatsApp/push | — | **BLOCKED BY EXTERNAL CREDENTIAL** (stubs retained) |
...
2. **Email / SMS / WhatsApp / Push provider credentials** — adapters present, stubs only — **BLOCKED BY EXTERNAL CREDENTIAL**
3. Production secrets — never ship demo `.env` / `Demo@12345`
